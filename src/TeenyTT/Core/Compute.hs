...
-- FIXME: This should live in Core.Compute
...
newtype CmpM a = CmpM { unCmpM :: ReaderT (Env (Maybe D.Value)) (Except Error) a }
    deriving (Functor, Applicative, Monad)

runCmp :: Env (Maybe D.Value) -> CmpM a -> Either Error a
...
getGlobal :: (MonadCmp m) => Level -> m (Maybe D.Value)
